-- View Example
CREATE OR REPLACE VIEW UniqueSubjectId AS
...
WHERE lower(ne.category)=lower('Nursing') and ie.expire_flag=0


+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#urine-output-all-day.sql

-- View Example
CREATE OR REPLACE VIEW uofirstday AS
select
  -- patient identifiers
...
group by ie.subject_id, ie.hadm_id, ie.icustay_id, date_trunc('day',oe.charttime)
+++++++++++++++++++++++++++++++++++++++++

+++++++++++++++++++++++++++++++
create or REPLACE VIEW ventdurations as
with vd0 as
(
  select
    icustay_id
...
select icustay_id, date_trunc('day',charttime) as charttime_day
  ...
  , (date_diff('second', max(charttime),min(charttime))/60/60) AS duration_hours
from vd2
group by icustay_id, ventnum, date_trunc('day',charttime)
...
+++++++++++++++++++++++++++++++++++++++++++
CREATE OR REPLACE VIEW ventfirstday AS
select
...
group by ie.subject_id, ie.hadm_id, ie.icustay_id, vd.charttime_day
order by ie.subject_id, ie.hadm_id, ie.icustay_id;

+++++++++++++++++++++++++++++++++++++++++++++++
create OR REPLACE view vitalsfirstday as
...
++++++++++++++++++++++++++++++++++++++++++++++++++
create OR REPLACE view gcsfirstday as
...
++++++++++++++++++++++++++++++++++++++++CREATE OR REPLACE VIEW OASIS as
...
select distinct ie.subject_id, ie.hadm_id, ie.icustay_id, vital.charttime
      ...
      , ROUND((date_diff('second',adm.admittime,ie.intime))/(60*60*24), 4) as preiculos
      ...
      , case
        when  
        ( ROUND((date_diff('second', pat.dob,ie.intime))/(60*60*24), 4) ) <= 1 then 'neonate'
        when ( ROUND((date_diff('second', pat.dob,ie.intime))/(60*60*24), 4) ) <= 15 then 'middle'
...
, case when preiculos is null then null
     when preiculos <  10 then 5
     when preiculos <  300 then 3
     when preiculos <  3600 then 0
     when preiculos <  43200 then 1
